...
from context import regions, intentions, tipologies
from utils import build_url


class AbitazioneScrapper:
    def __init__(self, base_url: str, max_num_pages: int = 80):
        """
    ...
    def paginate_urls(self) -> List[str]:
        """
...
fetching_urls = build_url(regions, intentions, tipologies)

results = []

for url in fetching_urls:
    sell_scrapper = AbitazioneScrapper(url)
    sell_results = sell_scrapper.main_scrapper()
    results.append(sell_results)


with open("../data/selling.json", "w") as f:
    json.dump(results, f)
